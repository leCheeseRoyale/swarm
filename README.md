...
Every swarm run is enforced by three hook layers:

```
┌──────────────────────────────────────────────┐
│  Layer 1: Plugin hooks (always active)       │
│  ├─ PreToolUse → guard-write.sh              │
│  │  Reads state.json, blocks code writes     │
│  │  outside active pipeline stages           │
│  └─ SubagentStop → completion check          │
│     Blocks orchestrator if tasks remain      │
├──────────────────────────────────────────────┤
│  Layer 2: Hookify rules (per-session)        │
│  ├─ swarm-guard: blocks session stop         │
│  ├─ swarm-safety: warns on destructive cmds  │
│  └─ swarm-context: injects state on prompt   │
│  Generated by /swarm:run, cleaned up after   │
├──────────────────────────────────────────────┤
│  Layer 3: Orchestrator agent                 │
│  └─ Reads team pipeline, dispatches agents   │
│     Updates state.json after every step      │
└──────────────────────────────────────────────┘
```

- **Layer 1** is deterministic — a bash script reads state.json and blocks or allows. No LLM involved.
- **Layer 2** is hot-reloadable — hookify rules activate instantly, no restart needed. Cleaned up when the swarm finishes.
- **Layer 3** is the coordinator — the orchestrator reads the team pipeline and dispatches the right agent for each stage.

Users never configure hooks manually. `/swarm:run` generates them from the team config, `/swarm:run` cleans them up when done.
